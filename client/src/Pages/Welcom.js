...
const Welcom = () => {
  ...
  const numberOfAnnounce= useSelector((state) => state.announceReducer.announce.numberOfAnnounce);
  let items = [];
for (let number = 1; number <= Math.ceil(numberOfAnnounce/limit); number++) {
  items.push(
   
    <PaginationItem key={number} page={number}
      style={{color:"white"}}
  ...
  const pubAnnounce = useSelector((state) => state.announceReducer.announce.announces);
  ...
  return (
    <>
      ...
      <h1 style={{color:"white",textAlign:"center",marginTop:"5%"}}>{numberOfAnnounce} Announces</h1>
...
{loadAnnounce ?<CircularProgress disableShrink />:pubAnnounce&&pubAnnounce.map((el,i)=>
  <PublicAnnounce announce={el} key={i} />)}   
      ...
      <div className="pagination-pubAnnounce">
      {items}
</div>
